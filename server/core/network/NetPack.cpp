...
{
    if (data.size() < (sizeof(int32_t) * 3 + sizeof(int64_t) * 2 + sizeof(int8_t)))
        return;

    const char *ptr = data.data();
    memcpy(&this->len, ptr, sizeof(int32_t));
    ptr += sizeof(int32_t);
    memcpy(&this->seq, ptr, sizeof(int32_t));
    ptr += sizeof(int32_t);
    memcpy(&this->msg_id, ptr, sizeof(int32_t));
    ptr += sizeof(int32_t);
    memcpy(&this->conn_id, ptr, sizeof(int64_t));
    ptr += sizeof(int64_t);
    memcpy(&this->uid, ptr, sizeof(int64_t));
    ptr += sizeof(int64_t);
    memcpy(&this->flag, ptr, sizeof(int8_t));
    ptr += sizeof(int8_t);
...
std::shared_ptr<std::string> NetPack::serialize() const
{
    ...
    int32_t total_len =
        sizeof(len) + sizeof(seq) + sizeof(msg_id) + sizeof(conn_id) + sizeof(uid) + sizeof(flag) + msg.size();
    out->resize(total_len);
    char *ptr = out->data();

    memcpy(ptr, &total_len, sizeof(len));
    ptr += sizeof(len);
    memcpy(ptr, &seq, sizeof(seq));
    ptr += sizeof(seq);
    memcpy(ptr, &msg_id, sizeof(msg_id));
    ptr += sizeof(msg_id);
    memcpy(ptr, &conn_id, sizeof(conn_id));
    ptr += sizeof(conn_id);
