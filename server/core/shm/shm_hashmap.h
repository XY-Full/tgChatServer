...
struct Spin
{
    ...
    void lock()
    {
        ...
        {
            uint32_t e = 0;
            if (s.compare_exchange_weak(e, 1, std::memory_order_acquire))
                return;
...
{
public:
    ShmHashMap(void *base, uint32_t total_bytes, bool create, uint32_t bucket_new = 4096, uint32_t lock_count = 64);

    // 线程/进程级：注册/注销读者槽（建议 TLS 保存 idx）
    int RegisterReader(uint32_t tid_hint = 0)
    {
        return epoch_.RegisterReader(tid_hint);
    }
    void UnregisterReader(int idx)
    {
        epoch_.UnregisterReader(idx);
    }
    ...
    bool RehashInProgress() const
    {
        return hdr_->rehash_active.load(std::memory_order_acquire) != 0;
    }

    // 统计
    uint32_t Size() const
    {
        return hdr_->item_count;
    }
...
private:
    ...
    inline void *off2ptr(uint32_t off) const
    {
        return off ? base8() + off : nullptr;
    }
    ...
    inline uint32_t *buckets_old() const
    {
        return reinterpret_cast<uint32_t *>(off2ptr(hdr_->buckets_old_off));
    }
    inline uint32_t *buckets_new() const
    {
        return reinterpret_cast<uint32_t *>(off2ptr(hdr_->buckets_new_off));
    }
    inline Spin *locks() const
    {
        return reinterpret_cast<Spin *>(off2ptr(hdr_->locks_off));
    }

    inline uint32_t lock_index(uint64_t h) const
    {
        return (uint32_t)(h & (hdr_->lock_count - 1));
    }
