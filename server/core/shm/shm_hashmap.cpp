...
ShmHashMap::ShmHashMap(void *base, uint32_t total_bytes, bool create, uint32_t bucket_new, uint32_t lock_count)
    : base_(base), total_(total_bytes),
      slab_(base, total_bytes,
            /*region_off*/ sizeof(HMHeader) + sizeof(Spin) * lock_count + sizeof(uint32_t) * bucket_new /*buckets_new*/
                + sizeof(uint32_t) * 0 /*buckets_old initially 0*/,
            /*region_size*/ total_bytes -
                (sizeof(HMHeader) + sizeof(Spin) * lock_count + sizeof(uint32_t) * bucket_new),
            create),
      epoch_(base, /*offset later set*/ 0, /*create=*/create) // 先占位，稍后修正
{
    if (!base || total_bytes < 1 << 20)
        throw std::runtime_error("ShmHashMap: shm too small");

    if (create)
    {
        if (!is_pow2(bucket_new))
        {
            uint32_t b = 8;
            while (b < bucket_new)
                b <<= 1;
...
void ShmHashMap::RetireNode(uint32_t node_off)
{
    ...
    do
    {
        head = hdr_->retire_head.load(std::memory_order_acquire);
        rec->next = head;
    }
    ...
    while (cur)
    {
            ...
            std::memcmp(reinterpret_cast<uint8_t *>(n) + sizeof(Node), key, klen) == 0)
        {
            ...
            else
            {
                ...
                if (!new_off)
                {
                    hdr_->puts_fail.fetch_add(1, std::memory_order_relaxed);
                    sp.unlock();
                    return false;
                }
                ...
                if (prev == 0)
                    *head = new_off;
                    ...
                    reinterpret_cast<Node *>(off2ptr(prev))->next.store(new_off, std::memory_order_relaxed);
                // 退休旧节点
                RetireNode(cur);
                hdr_->puts_overwrite.fetch_add(1, std::memory_order_relaxed);
                sp.unlock();
...
bool ShmHashMap::RehashStart(uint32_t new_bucket_count)
{
    ...
    if (new_off + sizeof(uint32_t) * new_bucket_count + 1 > total_)
        return false;
...
uint32_t ShmHashMap::MigrateOneBucket(uint32_t b)
{
    ...
    for (uint32_t i = 0; i < hdr_->lock_count; ++i)
        locks()[i].lock();
...
uint32_t ShmHashMap::RehashStep(uint32_t steps)
{
    ...
    while (steps--)
    {
        ...
        if (b >= hdr_->bucket_count_old)
        {
            hdr_->rehash_active.store(0, std::memory_order_release);
            break;
        }
