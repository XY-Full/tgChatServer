...
Timer::Timer() : running_(true), nextId_(1)
{
...
Timer::TaskId Timer::runEvery(double intervalSeconds, Callback cb)
{
    ...
    auto interval = std::chrono::duration_cast<Duration>(std::chrono::duration<double>(intervalSeconds));
...
void Timer::workerThread()
{
    ...
    while (running_)
    {
        ...
        {
            // 找到下一个最近要执行的任务
            auto nextIt = std::min_element(tasks_.begin(), tasks_.end(),
                                           [](auto &a, auto &b) { return a.second.nextRun < b.second.nextRun; });
            auto now = Clock::now();
