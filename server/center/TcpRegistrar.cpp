...
TcpRegistrar::~TcpRegistrar(){ stop(); }
...
void TcpRegistrar::accept_loop() {
    ...
    if (listen(listen_fd, 128) < 0) { perror("listen"); close(listen_fd); return; }
    ...
    while (!stop_) {
        ...
        if (client_fd < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) { std::this_thread::sleep_for(std::chrono::milliseconds(50)); continue; }
            ...
            try {
                ...
                if (j.contains("type") && j.at("type").get<std::string>() == "heartbeat") {
                    ...
                    if (inst && inst->id == id) {
                        ...
                        if (renewed) write(client_fd, std::string("OK ").data(), 3);
                        else write(client_fd, std::string("ERR no such id ").data(), 12);
                    }
                } else {
                    // 视为注册消息（兼容历史实现）
