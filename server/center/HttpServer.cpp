...
void HttpServer::start()
{
    ...
    svr.Post("/deregister", [&](const httplib::Request &req, httplib::Response &res) {
        ...
        {
            res.status = 400;
            res.set_content("{\"error\":\"invalid\"}", "application/json");
        }
    });
    ...
    svr.Get(R"(/list)", [&](const httplib::Request &req, httplib::Response &res) {
        ...
        {
            res.status = 400;
            res.set_content("{\"error\":\"service required\"}", "application/json");
            return;
        }
    ...
    svr.Get(R"(/discover)", [&](const httplib::Request &req, httplib::Response &res) {
        ...
        {
            res.status = 404;
            res.set_content("{\"error\":\"no instances\"}", "application/json");
            return;
        }
        ...
        {
            res.status = 500;
            res.set_content("{\"error\":\"no candidate\"}", "application/json");
            return;
        }
        json out;
        out["id"] = chosen->id;
        out["address"] = chosen->address;
    ...
    svr.Get(R"(/routing_table)", [&](const httplib::Request &req, httplib::Response &res) {
        res.status = 200;
        res.set_content(registry_.routing_table_string(), "text/plain");
    });
    ...
    svr.Get(R"(^/proxy/([A-Za-z0-9_.-]+)/(.+)$)", [&](const httplib::Request &req, httplib::Response &res) {
        ...
        {
            res.status = 400;
            res.set_content("invalid proxy path", "text/plain");
            return;
        }
        auto svc = req.matches[1];
        auto path = std::string("/") + req.matches[2].str();
        ...
        {
            res.status = 404;
            res.set_content("no instances", "text/plain");
            return;
        }
        auto lb = lb_factory_->create(strategy);
        auto chosen = lb->select(instances, key);
        ...
        {
            res.status = 500;
            res.set_content("no candidate", "text/plain");
            return;
        }
...
void HttpServer::handle_register(const std::string &body, std::string &out, int &status)
{
    try
    {
        ...
        if (j.contains("ttl"))
            ttl = std::chrono::seconds(j.at("ttl").get<int>());
