...
public:
    ServiceInstancePtr select(const ServiceInstances& instances, const std::string& key = "") override {
        ...
        for (auto &p: instances) {
            ...
            for (uint32_t i=0;i<w;i++) wheel.push_back(p);
        }
        if (wheel.empty()) return nullptr;
...
public:
    ServiceInstancePtr select(const ServiceInstances& instances, const std::string& key = "") override {
        ...
        for (auto &p : instances) {
            ...
            if (!best || c < best_conn) { best = p; best_conn = c; }
        }
        return best;
    }
};
...
public:
    ServiceInstancePtr select(const ServiceInstances& instances, const std::string& key = "") override {
        if (instances.empty()) return nullptr;
        if (key.empty()) return RoundRobinLB().select(instances);
...
public:
    ServiceInstancePtr select(const ServiceInstances& instances, const std::string& key = "") override {
        ...
        for (auto &p : instances) {
            ...
            if (it == nodes.end()) {
                Node n; n.weight = std::max<uint32_t>(1, p->weight); n.current = 0; n.effective_weight = n.weight;
                nodes[p->id] = n;
            } else {
                // 更新权重
        ...
        for (auto it = nodes.begin(); it != nodes.end();) {
            if (alive.find(it->first) == alive.end()) it = nodes.erase(it);
            else ++it;
        }
        // 选择最大 current
        std::string best_id;
        ...
        for (auto &p : instances) {
            ...
            if (n.current > best_current || best_id.empty()) { best_current = n.current; best_id = p->id; }
        }
        if (best_id.empty()) return nullptr;
        nodes[best_id].current -= total;
        // 返回对应实例指针（从传入的 instances 中查找）
        for (auto &p : instances) if (p->id == best_id) return p;
        return nullptr;
    }
private:
    struct Node { uint32_t weight; uint64_t current; uint32_t effective_weight; };
    std::mutex mu_;
...
public:
    ServiceInstancePtr select(const ServiceInstances& instances, const std::string& key = "") override {
        ...
        for (auto &p : instances) {
            ...
            if (lat == 0) lat = UINT64_MAX/2; // 未知延迟视为很高
            if (!best || lat < best_lat) { best = p; best_lat = lat; }
        }
        return best;
    }
};
...
void LBFactory::register_factory(const std::string& name, Creator c) { factory_[name]=c; }
std::shared_ptr<ILoadBalancer> LBFactory::create(const std::string& name) {
    auto it = factory_.find(name);
    if (it==factory_.end()) return factory_["round_robin"]();
    return it->second();
}